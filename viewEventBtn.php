<?php
    ...
    if(isset($_SESSION['userid'])){
        ...
        if($_SESSION['userid']==$user_id){
            ...
            $stmt = $mysqli->prepare("select event_descrip from events where event_id = ?");
            if(!$stmt){
                echo json_encode(array("status"=>"failure","error" => $mysqli->error));
                exit;
            }
            ...
            $stmt->execute();
            $stmt->bind_result($event_descrip);
            $stmt->fetch();
            $stmt->close();
            $_SESSION['event_descrip'] = htmlentities($event_descrip);
            echo json_encode(array("status"=>"success", "event_descrip"=>htmlentities($event_descrip)));
        }else{

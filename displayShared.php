...
$permission->bind_param('s', $shareFromUsername);
    ...
    $stmt = $mysqli->prepare("select event_name, event_descrip, event_day, event_month, event_year, event_hour, event_min, event_id, am_or_pm, category from events, users where (events.user_id = users.user_id && users.username=?)
    ...
    $stmt->bind_param('s', $shareFromUsername);
    $stmt->execute();
